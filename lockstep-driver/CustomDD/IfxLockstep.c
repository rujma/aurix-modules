...
void IfxLockstep_SMU_Init (void)
{
    ...
    IfxSrc_init(&SRC_SMU0, IfxSrc_Tos_cpu0, ISR_PRIORITY_SMU_INT0);
    /* Reload the register */
...
void IfxLockstep_testCheckerCore(IfxLockstep_CheckerNumber ProcessorNumber){
    ...
    sint32 reg = SCU_LCLTEST.U;
    IfxLockstep_CheckerNumber Value = (1 >> ProcessorNumber) & 0x03u;

    SCU_LCLTEST.U = (reg & 0xFFFFFFFCu) | Value;
}
